...
export class FeedbackService {
  ...
  async submitFeedback(
    ...
    if (!userId)
      throw new UnauthorizedException('Bạn phải đăng nhập để gửi phản hồi.');

    const prediction =
      await this.predictionHistoryModel.findById(prediction_id);
    if (!prediction)
      throw new NotFoundException('Không tìm thấy lịch sử dự đoán.');

    if (prediction.user?.toString() !== userId.toString())
      throw new BadRequestException('Bạn không có quyền đánh giá kết quả này.');

    const existingFeedback = await this.feedbackModel.findOne({
      prediction_id,
    });
    if (existingFeedback)
      throw new BadRequestException('Bạn đã gửi phản hồi cho kết quả này rồi.');
    ...
    if (prediction.source === 'image_upload' && file_path) {
      const from_public_id =
        file_path.substring(0, file_path.lastIndexOf('.')) || file_path;
      const to_public_id = `dataset/pending/${path.basename(from_public_id)}`;

      try {
        logger.info(
          `[Feedback Service] Moving Cloudinary resource from '${from_public_id}' to '${to_public_id}'`,
        );
        ...
        if (prediction.media) {
          await this.mediaModel.updateOne(
            { _id: (prediction.media as any)._id || prediction.media },
            { $set: { mediaPath: final_file_path_with_ext } },
          );
        }
        await this.predictionHistoryModel.updateOne(
          { _id: prediction._id },
          { $set: { mediaPath: final_file_path_with_ext } },
        );
      } catch (error: any) {
        logger.warn(
          `[Feedback Service] Could not move Cloudinary resource. Error: ${error.message}`,
        );
      }
    }

    const predictionAfterUpdate = await this.predictionHistoryModel.findById(
      prediction._id,
    );
    const final_path_for_feedback =
      predictionAfterUpdate?.mediaPath || file_path;
    ...
    if (user?.email) {
      this.mailService
        ...
        .catch((e) =>
          logger.error('Failed to send feedback thank you email:', e),
        );
    }
              ...
              { $match: { isDeleted: false } },
              {
                $group: {
                  ...
                  approved: {
                    $sum: {
                      $cond: [
                        { $eq: ['$status', 'approved_for_training'] },
                        1,
                        0,
                      ],
                    },
  ...
  async updateFeedback(
    id: string,
    data: { status: string; admin_id?: string; reason?: string },
    correctedLabel?: string,
  ) {
    const feedback = await this.feedbackModel
      .findById(id)
      .populate('prediction_id', 'predictions');
    if (!feedback)
      throw new NotFoundException('Không tìm thấy feedback để cập nhật.');

    feedback.status = data.status as any;
    if (data.admin_id)
      feedback.admin_id = new Types.ObjectId(data.admin_id) as any;
  ...
  async approveFeedback(
    ...
    if (feedback.status !== 'pending_review')
      throw new BadRequestException('Feedback này đã được xử lý.');
    ...
    const updatedFeedback = await this.updateFeedback(
      id,
      { status: 'approved_for_training', admin_id: adminId },
      correctedLabel,
    );
