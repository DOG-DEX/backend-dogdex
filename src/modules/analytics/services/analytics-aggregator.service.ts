...
export class AnalyticsAggregatorService {
  constructor(
    ...
    @InjectModel('AnalyticsSummary')
    private analyticsSummaryModel: Model<AnalyticsSummaryDoc>,
  ) {}

  public async runMonthlyRollup(): Promise<void> {
    logger.info(
      '[AnalyticsRollup] Starting monthly analytics aggregation job...',
    );
    ...
    logger.info(
      `[AnalyticsRollup] Aggregating data for ${targetMonth}/${targetYear}...`,
    );
    ...
    try {
      ...
      const monthlySummaries =
        await this.analyticsEventModel.aggregate(aggregationPipeline);

      if (monthlySummaries.length === 0) {
        logger.info(
          '[AnalyticsRollup] No data to aggregate for the last month.',
        );
        return;
      }
      ...
      logger.info(
        `[AnalyticsRollup] Successfully aggregated and saved ${monthlySummaries.length} summaries.`,
      );
      ...
      logger.info(
        `[AnalyticsRollup] Deleted ${deleteResult.deletedCount} old detailed analytics events.`,
      );
    } catch (error: any) {
      logger.error(
        '[AnalyticsRollup] Error during analytics aggregation job:',
        error.message,
      );
    }
  }
