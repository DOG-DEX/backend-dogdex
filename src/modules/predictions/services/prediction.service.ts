...
import { PredictionHistoryDoc, StreamResultPayload } from '../schemas/prediction_history.model';
...
export class PredictionService {
  ...
  private async getUserDirectoryId(userId: string): Promise<Types.ObjectId | undefined> {
  ...
  private async getModelName(): Promise<string> {
    const model = await this.aiModel.findActiveModelForTask('DOG_BREED_CLASSIFICATION');
    return model?.name ?? 'unknown_model';
  }

  private trackEvent(userId: string | undefined, req: Request, processingTime?: number) {
  ...
  async makePrediction(
    ...
    const directoryId = userId ? await this.getUserDirectoryId(userId) : undefined;
  ...
  async makeEphemeralPrediction(file: Express.Multer.File, userId?: string): Promise<any> {
    ...
    return { predictions: result.predictions, processed_media_base64: result.processed_media_base64 };
  }
  ...
  async makeBatchPredictions(
    ...
    const results = await Promise.all(
      files.map((file) => this.processSingleBatchFile(file, userId, directoryId, modelName)),
    );

    const predictions = results.filter((p): p is PredictionHistoryDoc => p !== null);
    this.trackEvent(userId, req);
    logger.info(`[PERF] BACKEND | batch | count: ${files.length} | total: ${Date.now() - startTime}ms`);
    return predictions;
  }
  ...
  private async processSingleBatchFile(
    ...
    try {
      ...
      const [aiResult, originalPath] = await Promise.all([
        ...
        this.cloudinary.uploadFile(file.path, `${filenameWithoutExt}_${Date.now()}`, 'public/uploads/images', 'image', 'private')
          .then((r) => `${r.public_id}.${r.format}`)
          .catch(() => file.filename),
      ]);

      if (!aiResult?.predictions || !aiResult?.processed_media_base64) throw new Error('Invalid AI result');
      ...
      const processedPath = await this.cloudinary.uploadBase64(
        aiResult.processed_media_base64, 'public/processed/images', 'image', 'private',
      );
  ...
  async saveStreamPrediction(
    ...
    const processedPath = await this.cloudinary.uploadBase64(
      payload.processed_media_base64, 'public/processed/images', 'image', 'private',
    );
    const newMedia = await this.mediaModel.create({
  ...
  async makeUrlPrediction(userId: string | undefined, url: string, req: Request): Promise<PredictionHistoryDoc> {
    const startTime = Date.now();
    ...
    if (!aiResult?.predictions) throw new Error('URL không hợp lệ hoặc không phải ảnh hợp lệ.');
  ...
  private async resolveUrl(url: string): Promise<string> {
    try {
      if (url.startsWith('data:image')) {
        return this.cloudinary.uploadBase64(url.split(',')[1], 'public/uploads/images', 'image', 'private');
      }
      const urlObj = new URL(url);
      if (urlObj.hostname.includes('google.com')) {
        return urlObj.searchParams.get('imgurl') || urlObj.searchParams.get('url') || url;
      }
    } catch { /* ignore */ }
    return url;
  }
  ...
  private async callAiWithUrl(url: string): Promise<any> {
    ...
    const response = await axios.post(`${process.env.AI_SERVICE_URL || 'http://localhost:8000'}/predict/url`, { url });
  ...
  async processAsyncPrediction(data: any): Promise<void> {
    const { predictionId, fileType, filePath, fileOriginalName, mediaId, userId, directoryId, modelName, startTime, analyticsData } = data;
    try {
      const buffer = fileType === 'image'
      ...
      await this.queueService.enqueueUpload({
        predictionId, mediaId, predictionHistoryId: predictionId,
        userId, directoryId, filePath, fileOriginalName, fileType,
        predictionResult: { predictions: aiResult.predictions },
        processedMediaPathTemp: tempPath,
        modelName, startTime, analyticsData,
      });
    } catch (err) {
      logger.error(`[AsyncPrediction] Failed:`, err);
      await this.historyModel.findByIdAndUpdate(predictionId, { processedMediaPath: 'failed' });
  ...
  async processBackgroundUpload(data: any): Promise<void> {
    const {
      predictionId, mediaId, predictionHistoryId, userId,
      filePath, fileOriginalName, fileType,
      startTime, analyticsData, processedMediaPathTemp,
    } = data;
    ...
    try {
      ...
      const [originalPath, processedPath] = await Promise.all([
        this.cloudinary.uploadFile(filePath, filenameWithoutExt, `public/uploads/${fileType}s`, fileType, 'private')
          .then((r) => `${r.public_id}.${r.format}`),
        this.cloudinary.uploadBase64(base64, `public/processed/${fileType}s`, fileType, 'private'),
      ]);

      await this.mediaModel.findByIdAndUpdate(mediaId, { mediaPath: originalPath, type: fileType });
      const processingTime = Date.now() - startTime;
      ...
      const reqMock = { ip: analyticsData.ip, headers: { 'user-agent': analyticsData.userAgent }, user: userId ? { _id: userId } : undefined } as any;
      this.analytics.trackEvent({ eventName: userId ? 'SUCCESSFUL_PREDICTION' : 'SUCCESSFUL_TRIAL', req: reqMock, processingTime });

      logger.info(`[BackgroundUpload] Done ${predictionId} in ${Date.now() - bgStart}ms`);
    } catch (err) {
      logger.error(`[BackgroundUpload] Failed ${predictionId}:`, err);
      await this.historyModel.findByIdAndUpdate(predictionHistoryId, { processedMediaPath: 'failed' });
    } finally {
      const { existsSync, promises } = await import('fs');
      if (existsSync(filePath)) await promises.unlink(filePath).catch(() => {});
      if (processedMediaPathTemp && existsSync(processedMediaPathTemp)) await promises.unlink(processedMediaPathTemp).catch(() => {});
    }
  }
}
