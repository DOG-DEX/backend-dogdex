...
export class MediaProcessorService {
  ...
  private initFfmpeg(): void {
    try {
      ...
      if (ffmpegPath) {
        ...
        logger.warn('[MediaProcessor] ffmpeg not found. Video optimization disabled.');
      }
      try {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
      ...
      logger.warn('[MediaProcessor] fluent-ffmpeg not installed. Video optimization disabled.');
    }
  }
  ...
  private probeFfmpeg(): string | null {
    ...
    if (envPath) {
      try { if (spawnSync(envPath, ['-version']).status === 0) return envPath; } catch { /* continue */ }
    }
    try { if (spawnSync('ffmpeg', ['-version']).status === 0) return 'ffmpeg'; } catch { /* continue */ }
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
  ...
  async optimizeVideo(filePath: string): Promise<Buffer> {
    if (process.env.SKIP_VIDEO_OPTIMIZATION === '1') {
      logger.warn('[MediaProcessor] SKIP_VIDEO_OPTIMIZATION — returning raw buffer');
      return fs.promises.readFile(filePath);
    }

    if (!this.ffmpegAvailable) {
      logger.warn('[MediaProcessor] ffmpeg not available, returning raw buffer.');
      return fs.promises.readFile(filePath);
    }
    ...
    try {
      ...
      logger.warn(`[MediaProcessor] FFmpeg failed (${err.message}). Falling back to raw.`);
      return fs.promises.readFile(filePath);
    }
  }
}
