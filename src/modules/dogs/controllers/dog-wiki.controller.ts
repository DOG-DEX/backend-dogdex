...
import { CreateDogBreedDto, UpdateDogBreedDto } from '../dto/dog-wiki.dto';
...
export class DogWikiController {
  ...
  async getTotalBreedsCount(@Query('lang') lang: string) {
    const count = await this.dogsWikiService.getTotalBreedsCount(
      (lang as 'vi' | 'en') || 'en',
    );
    return { total: count };
  }
  ...
  async getBreedBySlug(
    ...
    return this.dogsWikiService.getBreedBySlug(
      slug,
      (lang as 'vi' | 'en') || 'en',
    );
  }
  ...
  async createBreed(
    @Body() data: CreateDogBreedDto,
    @Query('lang') lang: string,
  ) {
    return this.dogsWikiService.createBreed(
      data,
      (lang as 'vi' | 'en') || 'en',
    );
  }
  ...
  async updateBreed(
    @Param('slug') slug: string,
    @Body() data: UpdateDogBreedDto,
    @Query('lang') lang: string,
  ) {
    return this.dogsWikiService.updateBreed(
      slug,
      data,
      (lang as 'vi' | 'en') || 'en',
    );
  ...
  async deleteBreed(@Param('slug') slug: string, @Query('lang') lang: string) {
    return this.dogsWikiService.softDeleteBreed(
      slug,
      (lang as 'vi' | 'en') || 'en',
    );
  }
